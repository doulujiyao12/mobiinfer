...
# 生成全部 6 个 visual chunk 的 W8A8 量化 OM 文件。
#
# 与 run_real_calib_16.sh 的区别 (W8A8 vs W8A16):
#   1) --act_bit 8 (激活量化到 8bit), 权重仍 --weight_bit 8 -> W8A8
#   2) PLATFORM=kirin9020 (与 run_real_calib_256_W8A8.sh 一致)
#   3) route 目录带 W8A8 标记, 避免与 W8A16 结果混淆
#
# DDK 工具链: 6.1.1.0
...
#     -> visual_plugin_quant_matmul_route.py (每 chunk, --num_samples 16, --act_bit 8)
#     -> run_visual_plugin_matmul_omc.sh    (/temp/fdh/model_omc_fdh/model_..._chunk_W8A8_{i}_real16_w8a8/)
...
echo "=========================================="
echo "  真实图像校准量化 (W8A8, 16 样本, DDK 6.1.1.0)"
...
echo ""
echo "全部 6 个 chunk 的 OM 已用真实校准输入 (W8A8, 16 张图, seq_len=608, DDK 6.1.1.0) 生成完毕。"
