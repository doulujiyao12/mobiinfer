...
# 生成全部 6 个 visual chunk 的 W4A16 量化 OM 文件。
#
...
#   - act_bit=16 (signed)
#   - 对应 README 顶部默认路线: Quant_act_weight_eco + W4 + group128 + A16(signed)
#
# DDK 工具链: 6.1.1.0 (量化工具 dopt)
# OMG 编译  : run_visual_plugin_matmul_omc.sh (--target=om -> 产物 .om)
# 平台      : kirin9030
# 校准样本  : 16 (calib_inputs_256 中每 16 取 1, 共 16 张/样本)
# 复用已生成的 npz (跳过 dump / bin->npz 步骤), 直接做量化+导出OMC
#
...
#     -> run_visual_plugin_matmul_omc.sh    (/temp/fdh/model_omc_fdh/model_..._chunk_W4A16_{i}_real16_w4a16/)
...
OMC_OUT_ROOT=/temp/fdh/model_omc_fdh_9030_w4a16
ROUTE_SUFFIX=real16_w4a16       # route_dir 后缀
OM_COLLECT_DIR=${OMC_OUT_ROOT}/model_visual_plugin_matmul_W4A16_${ROUTE_SUFFIX}   # 汇总目录
# ====================================================================
...
echo "=========================================="
echo "  真实图像校准量化 (W4A16, 16 样本, DDK 6.1.1.0)"
...
echo "[step 5] 逐 chunk 跑 量化+导出ONNX+OMC (W4A16, num_samples=${NUM_SAMPLES})"
echo ""
...
for i in 0 1 2 3 4 5; do
    ROUTE_DIR="${OMC_OUT_ROOT}/model_visual_plugin_matmul_chunk_W4A16_${i}_${ROUTE_SUFFIX}"
...
ALL_OK=1
for i in 0 1 2 3 4 5; do
    OM_FILE="${OMC_OUT_ROOT}/model_visual_plugin_matmul_chunk_W4A16_${i}_${ROUTE_SUFFIX}/omc_output/visual_plugin_matmul_quantized.om"
    if [ -f "${OM_FILE}" ]; then
...
for i in 0 1 2 3 4 5; do
    SRC_OM="${OMC_OUT_ROOT}/model_visual_plugin_matmul_chunk_W4A16_${i}_${ROUTE_SUFFIX}/omc_output/visual_plugin_matmul_quantized.om"
...
echo ""
echo "全部 6 个 chunk 的 OM 已用真实校准输入 (W4A16, 16 张图, seq_len=608, DDK 6.1.1.0) 生成完毕。"
