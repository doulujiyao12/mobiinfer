...
|inMaxTensor|输入|源操作数。 类型为[LocalTensor](./cannkit-localtensor.md)，支持的TPosition为VECIN、VECCALC、VECOUT。 softmax计算所需要的max值。 除模板参数config配置为非拓展模式（SoftmaxMode::SOFTMAX_OUTPUT_WITHOUT_BRC）的场景外，inMaxTensor的last轴长度固定为32Byte，即一个datablock长度。该datablock中的所有数据为同一个值，比如half数据类型下，该datablock里的16个数均为相同的值。 非last轴的长度需要与dstTensor保持一致。|
