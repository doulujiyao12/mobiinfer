...
2. 获取日志文件。
   ...
   通过命令行方式，日志落盘地址由[Simulator仿真参数](./cannkit-cli-parameters.md)接口指定，缺省为当前操作路径的debug_op.log。请根据实际路径打开日志文件。
3. 截取调测命令，重新执行后再分析。
...
2. 获取日志文件。
   ...
   通过命令行方式，日志落盘地址由[NPU调测参数](./cannkit-cli-parameters.md)接口指定，缺省为当前操作路径的debug_op.log。请根据实际路径打开日志文件。
...
* 建议4：请参考[Simulator仿真参数](./cannkit-cli-parameters.md)设置CAModel仿真参数，如block num建议设置为1。
