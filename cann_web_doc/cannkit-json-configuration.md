...
* 标准自定义算子工程场景的开发人员一般按此方式配置算子json文件。
...
* 场景3：支持原地算子格式的输入。
  ...
  当算子为[原地算子(in-place op)](./cannkit-commissioning-tools.md)时，输入和输出地址一样，配置算子json时"outputs"和"inputs"除了"data_file"不同，其他配置项均保持一致。配置示例如下。
