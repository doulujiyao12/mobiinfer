...
提供[GET_TILING_DATA](./cannkit-get-tiling-data.md)，用于获取算子kernel入口函数传入的tiling信息，并填入注册的Tiling结构体中，此函数会以宏展开的方式进行编译。注意，对应的算子host实现中需要定义TilingData结构体，实现并注册计算TilingData的Tiling函数。具体请参考[Host侧Tiling实现](./cannkit-tiling-implementation-on-the-host.md)。

核函数中调用[GET_TILING_DATA](./cannkit-get-tiling-data.md)获取TilingData的样例如下。
...
![](https://media:401788444102453872)  
![](https://media:401788444102476873)  
