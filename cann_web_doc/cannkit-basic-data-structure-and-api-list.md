...
|类和结构体|[StorageShape](./cannkit-storageshape-introduction.md)|该类描述了tensor的shape，包含两个信息：origin_shape以及storage_shape。|
...
|分类|数据结构/接口名称|功能描述|
|:----|:-----------------------------------------------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|类和结构体|[Allocator](./cannkit-allocator-construction-and-destructor.md)|支持使用开发者注册的外置allocator功能，所在头文件位于CANN软件安装后文件存储路径下的"include/ge/ge_allocator.h"路径。|
|类和结构体|[AscendString](./cannkit-ascendstring-construction-and-destructor.md)|用于存储字符串。 - 头文件位于CANN软件安装后文件存储路径下的include/graph/ascend_string.h - 库文件：libgraph.so|
|类和结构体|[AttrValue](./cannkit-attrvalue-construction-and-destructor.md)|用于存储属性值。 - 头文件位于CANN软件安装后文件存储路径下的include/graph/attr_value.h - 库文件：libgraph_base.so|
...
|类和结构体|[MemBlock](./cannkit-memblock-construction-and-destructor.md)|配合[Allocator](./cannkit-allocator-construction-and-destructor.md)类使用，支持使用开发者注册的外置allocator功能，所在头文件位于CANN软件安装后文件存储路径下的"include/ge/ge_allocator.h"路径。|
...
|类和结构体|[Shape](./cannkit-shape-construction-and-destructor.md)|用于存储Tensor的shape信息。 - 头文件位于CANN软件安装后文件存储路径下的include/graph/tensor.h - 库文件：libgraph_base.so|
|类和结构体|[TensorDescInfo](./cannkit-tensordescinfo.md)|存储Tensor描述信息。|
|类和结构体|[TensorDesc](./cannkit-tensordesc-construction-and-destructor.md)|用于存取、管理Tensor描述信息。 - 头文件位于CANN软件安装后文件存储路径下的include/graph/tensor.h - 库文件：libgraph_base.so|
|类和结构体|[TensorType](./cannkit-tensortype.md)|TensorType类用以定义输入或者输出支持的数据类型。|
|类和结构体|[Tensor](./cannkit-tensor-construction-and-destructor.md)|Tensor结构。 - 头文件位于CANN软件安装后文件存储路径下的include/graph/tensor.h - 库文件：libgraph_base.so|
...
|函数|[GetSizeByDataType](./cannkit-getsizebydatatype.md)|根据传入的data_type，获取该data_type所占用的内存大小。|
|函数|[GetSizeInBytes](./cannkit-getsizeinbytes.md)|根据传入的element_count和data_type，获取element_count个该data_type所占用的内存总大小。|
...
|宏|[BROADCAST_INFER](./cannkit-broadcast-infer.md)|提供公共函数宏封装，供算子开发者开发InferShape函数。该函数基于2个输入的shape，设置输出的shape。该宏只是设置shape，未设置dtype。|
|宏|[COMMON_INFER_FUNC_REG](./cannkit-common-infer-func-reg.md)|注册算子的InferShape函数。|
|宏|[DECLARE_ERRORNO](./cannkit-declare-errorno.md)|错误码及描述注册宏。|
|宏|[ELMTWISE_INFER_SHAPEANDTYPE](./cannkit-elmtwise-infer-shapeandtype.md)|提供公共函数宏封装，供算子开发者开发InferShape函数。该函数基于输入的shape和dtype，设置输出的shape和dtype。|
|宏|[IMPLEMT_COMMON_INFERFUNC](./cannkit-implemt-common-inferfunc.md)|封装算子的Common_InferShape函数。|
|宏|[IMPLEMT_INFERFORMAT_FUNC](./cannkit-implemt-inferformat-func.md)|封装算子的inferFormat函数。|
|宏|[IMPLEMT_INFERFUNC](./cannkit-implemt-inferfunc.md)|封装算子的InferShape函数。|
|宏|[IMPLEMT_VERIFIER](./cannkit-implemt-verifier.md)|封装算子的Verify函数。|
|宏|[INFER_FORMAT_FUNC_REG](./cannkit-infer-format-func-reg.md)|注册算子的InferFormat实现。|
|宏|[INFER_FUNC_REG](./cannkit-infer-func-reg.md)|注册算子的InferShape函数。|
|宏|[原型定义接口（REG_OP）](./cannkit-prototype-definition-api.md)|原型定义接口。|
|宏|[原型定义衍生接口说明](./cannkit-prototype-definition-derivative-api.md)|原型定义衍生接口。|
|宏|[VERIFY_FUNC_REG](./cannkit-verify-func-reg.md)|注册算子的Verify函数。|
