...
|参数|输入/输出|说明|
|:-----------|:----|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|parseParamFn|输入|解析算子属性的函数ParseParamFunc，请参见[回调函数ParseParamFunc](#回调函数parseparamfunc)。 针对TensorFlow框架，若原始TensorFlow框架算子属性与适配AI处理器的模型中算子属性一一对应（属性个数与顺序一致），可直接使用[AutoMappingFn](./cannkit-automappingfn.md)函数自动实现映射。|
...
|op_dest|输出|适配AI处理器的模型的算子数据结构，保存算子信息。 关于Operator类，请参见[Operator](./cannkit-operator-construction-and-destructor.md)。|
