...
|CCE指令流打印|默认开启，CCE指令流文件参见[产物说明](#产物说明)。|
...
2. 执行如下命令进行CPU调测，这里仅提供关键参数项示例，其他参数请参考[CPU调测参数](./cannkit-cli-parameters.md)按需设置。
...
若开启[DumpTensor功能](./cannkit-commissioning-function-dumptensor.md)或[DumpAccChkPoint功能](./cannkit-commissioning-function-dumpaccchkpoint.md)，结果文件存放在dump目录下，结果目录具体介绍参见[产物说明](./cannkit-commissioning-function-dumptensor.md)。  
