...
![](https://media:401788444117546965)
...
* A_TYPE、B_TYPE、C_TYPE类型信息通过[使用说明](./cannkit-matmul-usage-description.md)表1来定义。
...
|ISTRANS|是否开启使能矩阵转置的功能。当前不支持转置，只支持设为false。 false为不开启使能矩阵转置的功能，通过[SetTensorA](./cannkit-settensora.md)和[SetTensorB](./cannkit-settensorb.md)不能设置A、B矩阵是否转置。Matmul会认为A矩阵形状为\[M, K\]，B矩阵形状为\[K, N\]。 默认为false不使能转置。|
...
|IterateOrder|Matmul做矩阵运算的循环迭代顺序，与[TCubeTiling结构体](./cannkit-tcubetiling-structure.md)表1中的iterateOrder参数含义相同。当ScheduleType参数取值为ScheduleType::OUTER_PRODUCT或1时，本参数生效。参数取值如下。 - ORDER_M = 0, // 先往M轴方向偏移再往N轴方向偏移 - ORDER_N, // 先往N轴方向偏移再往M轴方向偏移 - UNDEF, // 当前无效 说明： MDL模板使用时，若IterateOrder取值ORDER_M，[TCubeTiling结构体](./cannkit-tcubetiling-structure.md)表1中的stepN需要大于1，IterateOrder取值ORDER_N时，TCubeTiling结构中的stepM需要大于1。|Norm、MDL|
...
|cfg|[参数说明](#参数说明)下的表2|Matmul模板的参数配置。|
