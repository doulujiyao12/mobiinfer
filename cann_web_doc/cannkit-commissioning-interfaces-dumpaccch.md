...
基于算子工程开发的算子，可以使用该接口Dump指定Tensor的内容。同时支持打印自定义的附加信息（仅支持uint32_t数据类型的信息），比如打印当前行号等。区别于[DumpTensor](./cannkit-commissioning-interfaces-dumptensor.md)，使用该接口可以支持指定偏移位置的Tensor打印。
