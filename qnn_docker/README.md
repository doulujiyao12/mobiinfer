...
- 本镜像 `mnn-qnn:22.04` 用于在 x86_64 上运行 MNN + Qualcomm QNN SDK 相关工具。
- 约定路径：原始 QNN SDK 挂载到 `/opt/qnn`（只读），MNN 源代码挂载到 `/workspace/MNN`，QNN mirror 放在 `/root/qnn-mirror`。
...
```bash
docker build -t mnn-qnn:22.04 -f Dockerfile.mnn_qnn .
```

### 2. 交互式运行容器

```bash
docker run --rm -it --name mnn-qnn \
  -v /path/to/MNN:/workspace/MNN \
  -v /path/to/qnn-sdk:/opt/qnn:ro \
  -e BUILD_QNN_MIRROR=0 \
  mnn-qnn:22.04 /bin/bash
```

### 3. 启动时自动构建 QNN mirror

镜像启动后会把 `/opt/qnn` 复制到 `/root/qnn-mirror`，并对 QNN 的 `Makefile` 应用单行 patch：

```bash
docker run --rm -it --name mnn-qnn-build \
  -v /path/to/MNN:/workspace/MNN \
  -v /path/to/qnn-sdk:/opt/qnn:ro \
...
```bash
export LD_LIBRARY_PATH=/workspace/MNN/build_qnn_x86:$LD_LIBRARY_PATH
```
...
```bash
cd /workspace/MNN/build_qnn_x86
./MNN2QNNModel /opt/qnn 69 79 \
  /workspace/MNN/transformers/llm/export/model/visual_blocks.mnn \
  /workspace/MNN/transformers/llm/export/model/qnn_visual \
